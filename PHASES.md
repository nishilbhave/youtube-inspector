...
Master runbook for building `youtube-verdict` (V1 skill) inside the `youtube-inspector` repo. Each phase has a status, deliverables, verification gate, and a copy-paste prompt for Claude Code.
...
| 4 | Cross-platform validation + skills.sh publish | `[-]` Superseded by [`PUBLISH.md`](./PUBLISH.md) | 3 | Folded into 4-skill suite publish path; see `PUBLISH.md` |
...
- [x] `.gitignore` — `.venv/`, `__pycache__/`, `*.pyc`, `.DS_Store`, `~/youtube-reports/.cache/` reference, `.pytest_cache/`
...
Create:
- .gitignore (Python defaults: .venv/, __pycache__/, *.pyc, .pytest_cache/, .DS_Store, plus a comment that ~/youtube-reports/.cache/ is also ignored since it lives outside the repo)
...
- README.md: project description (umbrella for YouTube analysis skills), sister-skill roadmap as a table (youtube-verdict, youtube-claims, youtube-tldr, youtube-extract, youtube-channel, youtube-quote, youtube-clip), placeholder for install instructions
...
- [x] `--cache` flag round-trips correctly — verified live: rejection JSON written to `~/youtube-reports/.cache/{video_id}.json` (perms `0700`), second run serves from cache in ~0.03s vs 1.6–3.0s fresh (~50× speedup), no network call.
...
```
Implement Phase 2 — three prompts for youtube-verdict.
...
Caching is the agent's job. SKILL.md instructs it: before each pass, check the relevant cache file at `~/youtube-reports/.cache/{video_id}-pass{n}.json`. If present and the prompt hash matches, reuse. Otherwise run the pass and write the cache.

**Deliverable checklist:**
- [x] `skills/youtube-verdict/SKILL.md` with YAML frontmatter (name, description) — see Phase 4 for description tuning
...
- [x] Final report written to `~/youtube-reports/{video_id}.md` per the format in `yt-worth-it-plan.md`
...
- [x] End-to-end smoke on 3 transcripts from Phase 2: run inside Claude Code following SKILL.md as host agent; reports for `n0phBDPz8z0` (OKAY 5/10, 6 flags), `ru7fWKD4cyw` (OKAY 5/10, 6 flags), `erEgovG9WBs` (WATCH 9/10, 0 flags) match Phase 2 samples — IDENTICAL drift, all 12 flags verbatim against transcript segments.

**Verification:**
- Run the skill against 3 known transcripts inside Claude Code; outputs land in `~/youtube-reports/` and match Phase 2 samples within iteration drift.
- Delete only `~/youtube-reports/.cache/{video_id}-pass3.json` and re-trigger; confirm the agent reuses Pass 1 and Pass 2 caches and only re-runs Pass 3.
- Confirm no `import anthropic` / `import openai` / vendor SDK anywhere in the shipped repo.

**Prompt to submit to your agent:**

```
Implement Phase 3 — SKILL.md orchestration — for youtube-verdict.
...
Build skills/youtube-verdict/SKILL.md:
- YAML frontmatter: name=youtube-verdict, description=(placeholder; tuned in Phase 4)
- Body: numbered workflow the host agent follows when triggered
  ...
  3. Check cache for Pass 1 at ~/youtube-reports/.cache/{video_id}-pass1.json — reuse if prompt hash matches; else apply prompts/extract_structure.md to the transcript and write to cache
  4. Same pattern for Pass 2 (input: Pass 1 output + transcript) and Pass 3 (input: Pass 1 + Pass 2 + metadata)
  5. Write final report to ~/youtube-reports/{video_id}.md per the format in yt-worth-it-plan.md
...
Verify:
- Run youtube-verdict against 3 of the Phase 2 sample transcripts inside Claude Code (or any installed host agent). Outputs at ~/youtube-reports/{video_id}.md match the Phase 2 samples within iteration drift.
- Delete only ~/youtube-reports/.cache/{video_id}-pass3.json; re-run; confirm Passes 1–2 are NOT re-executed (the agent should report it skipped them due to cache hits).
...
> **Superseded by [`PUBLISH.md`](./PUBLISH.md).** The verdict-only deliverables in this section have been folded into the 4-skill suite publish path (`youtube-verdict` + `youtube-summary` + `youtube-extract` + `youtube-claims`). The trigger-quality tests, zero-setup smoke test, output-parity check, and `npx skills add` install verification still apply but now cover all four skills, not just verdict. **Do not work from this checklist** — use `PUBLISH.md` instead. This section remains for historical context.

**Status:** `[-]` Superseded by `PUBLISH.md`  
**Blocked by:** Phase 3
...
**Locked cross-platform test pair:** **Claude Code + Cursor.** Cursor is the largest non-Claude agent in the skills ecosystem; if youtube-verdict installs and runs the same way on both, the cross-platform claim holds for Antigravity / Codex / future agents that follow the same convention.
...
- [ ] **Zero-setup smoke test on Cursor:** uninstall youtube-verdict, clear `~/youtube-reports/`, install via `npx skills add ...`, paste a Phase 2 sample URL. The skill must produce a verdict report with **no prompts asking for keys, env vars, or configuration**. Document the full transcript of the install-to-output flow.
- [ ] **Output parity check:** run youtube-verdict on the same URL in Claude Code and Cursor. Acceptance: same WATCH/OKAY/SKIP verdict, same flag count ±1, same evidence-quality bar (verbatim quotes + timestamps).
...
- [ ] `npx skills add <owner>/youtube-inspector --skill youtube-verdict` install command verified end-to-end on a clean machine
...
```
Implement Phase 4 — cross-platform validation and skills.sh publish prep — for youtube-verdict.
...
ZERO-SETUP HARD RULE: A fresh user on Cursor must install youtube-verdict and produce a verdict on their first URL without:
...
Tune:
- skills/youtube-verdict/SKILL.md description (frontmatter): 80–120 words; concrete trigger phrases for "is this video worth watching", "what's actually in this video", "should I watch this", "skip or watch", "pre-watch summary", "is this YouTube video any good".
- README.md:
  - Install command: npx skills add <owner>/youtube-inspector --skill youtube-verdict
...
- Uninstall any prior install of youtube-verdict; clear ~/youtube-reports/
- Install via: npx skills add <owner>/youtube-inspector --skill youtube-verdict
...
- Acceptance: same WATCH/OKAY/SKIP, same flag count ±1, same evidence quality (verbatim quotes + timestamps)
...
- `[-]` Sister-skill priority order: which of `youtube-claims` / `youtube-summary` / `youtube-extract` ships next? — *Resolved 2026-05-05: all three built and shipping together as the 4-skill suite. See `PUBLISH.md`.*
...
| 2026-05-05 | 1 | Live e2e against 2 real URLs: `n0phBDPz8z0` (228s) → `TOO_SHORT` rejected at 300s floor; `xP0SQHXVHjQ` (Hindi) → `NON_ENGLISH` rejected. Cache round-trip verified live: rejection JSON cached at `~/youtube-reports/.cache/`, second run ~0.03s vs 1.6–3.0s fresh (~50× speedup). |
...
| 2026-05-05 | 4 | **4-skill suite assembled; Phase 4 superseded by `PUBLISH.md`.** Three sister skills written and smoke-tested alongside `youtube-verdict`: `youtube-summary` (factual neutral summary, `prompts/summarize_sections.md` + `prompts/generate_summary.md`; originally built as `youtube-tldr` and renamed mid-session for naming-register consistency with verdict/extract/claims — formal English nouns instead of internet shorthand), `youtube-extract` (categorized artifact list, `prompts/extract_artifacts.md` + `prompts/generate_extract.md`), `youtube-claims` (chronological claim inventory, shares Pass 2 with verdict via `prompts/inventory_claims.md`, plus `prompts/generate_claims.md`). New shared script `scripts/cache.py` ships the canonical hashing helper (`hash-file <path>`, `hash-json` from stdin) so SKILL.md cache wrappers no longer rely on inline `python3 -c` snippets that drift across hosts; `tests/test_cache.py` locks the algorithm with 22 tests. Fix #3: `python` → `python3` across all four SKILL.md subprocess calls. New `scripts/doctor.py` + `tests/test_doctor.py` (5 tests) — pre-flight deps check that prints the exact `pipx install yt-dlp youtube-transcript-api` command on `ModuleNotFoundError`. SKILL.md description audit: trigger-phrase collision `"what's actually in this video"` moved from `youtube-verdict` → `youtube-summary` (verdict was claiming a phrase that belonged to the neutral-summary skill). Rename details: `skills/youtube-tldr/` → `skills/youtube-summary/` (`git mv`), `prompts/generate_tldr.md` → `prompts/generate_summary.md` (`git mv`), report-filename suffix `-tldr.md` → `-summary.md`, cache filenames `{video_id}-tldr-pass{2,3}.json` → `{video_id}-summary-pass{2,3}.json`. "tl;dr" kept as a trigger phrase; "TL;DR" kept as the report's section-header format label. README rewritten in codeprobe style with banner SVG, four shields, sample-output dashboard, single-command install, and the 4-skill bullet list. Phase 4 verdict-only deliverables marked superseded; ongoing publish work tracked in `PUBLISH.md`. Tests: 121 → 126 passing (35 fetch + 30 segments + 22 cache + 34 slug + 5 doctor). Vendor-purity grep clean across `skills/`, `scripts/`, `prompts/`, `README.md`, `PHASES.md`, `PUBLISH.md`. |
| 2026-05-05 | 1+2 | **Phase 2 complete; Phase 1 happy-path verification folded in.** Fetched 12 real English-language YouTube transcripts (target was 10, corpus expanded after some user-supplied URLs duplicated existing picks): `W6NZfCO5SIk` Mosh JS course (48m, tutorial), `erEgovG9WBs` Fireship 100+ Web Dev (13m, tutorial), `3qHkcs3kG44` JRE #1309 Naval (132m, podcast >1hr), `n0phBDPz8z0` Travis Nicholson Lazy AI Money (4m, finance pitch), `ru7fWKD4cyw` Jono Catliff Claude Code $1.2M (77m, finance pitch long), `eRS3CmvrOvA` Nate Herk Claude Code Skills review (14m, tutorial review), `SVTPv4sI_Jc` Veritasium quantum sensor (21m, news/explainer), `L9ub_B71U0E` StarTalk wave-particle (13m, podcast/explainer), `6TXvaWX5OFk` FloatHeadPhysics quantum uncertainty (21m, tutorial/explainer), `ECOazagKKTo` Albert Olgaard AI notetaker (11m, tutorial/pitch), `4Qw4kyW8Ux8` Richard Yu build app with AI (20m, tutorial/finance pitch), `rIuv8mmshsY` Sahil & Sarra Code on a Notebook (9m, tutorial). Wrote three model-agnostic prompts: `prompts/extract_structure.md` (Pass 1, segmentation), `prompts/inventory_claims.md` (Pass 2, claim inventory; v2 patched for YouTube auto-caption sliding-window overlap), `prompts/generate_verdict.md` (Pass 3, synthesis with explicit FLAGS section enforcing the timestamp+quote citation rule). Pass 1 ran on all 12 transcripts (1 round, no rework, all outputs contiguous + valid JSON). Pass 2 + Pass 3 ran on 3 representative transcripts (Travis Nicholson, Jono Catliff, Fireship). Hard rule satisfied: 12/12 Pass-3 flags trace to exact (timestamp, verbatim quote) pairs in Pass 2 by independent substring audit. Verdicts: Fireship WATCH 9/10 (Gap LOW), Travis OKAY 5/10 (Gap MEDIUM, 6 flags), Jono OKAY 5/10 (Gap MEDIUM, 6 flags). Most common failure mode discovered and fixed: Pass 2's "join consecutive segments" rule produced non-verbatim quotes because YouTube auto-captions overlap; v2 switched to single-segment quotes only. Cross-model spot-check skipped per user direction in auto mode; deferred to Phase 4. Vlog category not in corpus (user-supplied URLs concentrated in tutorial / explainer / pitch hybrids); flagged for Phase 4 follow-up. `pytest -q` still 35 passing. |
