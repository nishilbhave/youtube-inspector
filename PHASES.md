...
> **Superseded by [`PUBLISH.md`](./PUBLISH.md).** The verdict-only deliverables in this section have been folded into the 4-skill suite publish path (`youtube-verdict` + `youtube-tldr` + `youtube-extract` + `youtube-claims`). The trigger-quality tests, zero-setup smoke test, output-parity check, and `npx skills add` install verification still apply but now cover all four skills, not just verdict. **Do not work from this checklist** — use `PUBLISH.md` instead. This section remains for historical context.
...
- `[ ]` Sister-skill priority order: which of `youtube-claims` / `youtube-tldr` / `youtube-extract` ships next?
...
| 2026-05-05 | 4 | **4-skill suite assembled; Phase 4 superseded by `PUBLISH.md`.** Three sister skills written and smoke-tested alongside `youtube-verdict`: `youtube-tldr` (factual neutral summary, `prompts/summarize_sections.md` + `prompts/generate_tldr.md`), `youtube-extract` (categorized artifact list, `prompts/extract_artifacts.md` + `prompts/generate_extract.md`), `youtube-claims` (chronological claim inventory, shares Pass 2 with verdict via `prompts/inventory_claims.md`, plus `prompts/generate_claims.md`). New shared script `scripts/cache.py` ships the canonical hashing helper (`hash-file <path>`, `hash-json` from stdin) so SKILL.md cache wrappers no longer rely on inline `python3 -c` snippets that drift across hosts; `tests/test_cache.py` locks the algorithm with 22 tests. Fix #3: `python` → `python3` across all four SKILL.md subprocess calls. New `scripts/doctor.py` + `tests/test_doctor.py` (5 tests) — pre-flight deps check that prints the exact `pipx install yt-dlp youtube-transcript-api` command on `ModuleNotFoundError`. SKILL.md description audit: trigger-phrase collision `"what's actually in this video"` moved from `youtube-verdict` → `youtube-tldr` (verdict was claiming a phrase that belonged to the neutral-summary skill). README rewritten in codeprobe style with banner SVG, four shields, sample-output dashboard, single-command install, and the 4-skill bullet list. Phase 4 verdict-only deliverables marked superseded; ongoing publish work tracked in `PUBLISH.md`. Tests: 121 → 126 passing (35 fetch + 30 segments + 22 cache + 34 slug + 5 doctor). Vendor-purity grep clean across `skills/`, `scripts/`, `prompts/`, `README.md`, `PHASES.md`, `PUBLISH.md`. |
